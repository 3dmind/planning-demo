...
type Response = Either<GetAllArchivedTasksErrors.MemberNotFoundError | AppErrors.UnexpectedError, Result<Task[]>>;
...
export class GetAllArchivedTasksUsecase implements UseCase<Request, Response> {
  ...
  async execute(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (!found) {
        const memberNotFoundError = new GetAllArchivedTasksErrors.MemberNotFoundError(request.userId);
